<?php
   $servername = "localhost";
   $username = "savannah";
   $password = "savannah"; // so secure
   $connection = mysqli_connect($servername, $username, $password);
   // check connection
   if (!$connection):
   echo("Connection failed: " . mysqli_connect_error());
   endif;

   $db = new mysqli($servername, $username, $password, 'cookbook');
   if ($db->connect_error):
   die ("Could not connect to db " . $db->connect_error);
   endif;

   $recipe_title ="a";
   $recipe_rating ="1";
   $recipe_directions ="a";
   $recipe_ingredients ="a";

   // get menu name
   if( $_REQUEST["m_name"] ):
      $menu_name = $_REQUEST["m_name"];
   endif;
   // get recipe title
   if( $_REQUEST["r_title"] ):
      $recipe_title = $_REQUEST["r_title"];
   endif;
   // get recipe rating
   if( $_REQUEST["r_rating"] ):
       $recipe_rating = $_REQUEST["r_rating"];
    endif;
    // get recipe directions
    if( $_REQUEST["r_dir"] ):
      $recipe_directions = $_REQUEST["r_dir"];
    endif;
    // get recipe ingredients
    if( $_REQUEST["r_ingred"] ):
      $recipe_ingredients = $_REQUEST["r_ingred"];
      $recipe_ingredients = str_replace(',', '#', $recipe_ingredients)."#";
    endif;
    // add the new recipe to the database
   $query = "insert into recipes values (NULL, '$recipe_title', '$recipe_rating', '$recipe_directions', '$recipe_ingredients')";
   $db->query($query) or die ("Invalid insert " . $db->error);

   // get the id of that recipe just added
   $query = "select * from recipes where recipes.title = '$recipe_title'";
   $result = $db->query($query) or die ("recipe not found");
   $recipe_id_result = $result->fetch_array();
   $recipe_id = stripslashes($recipe_id_result["id"]);

   // get the id of the current menu
   $query = "select * from menus where menus.name = '$menu_name'";
   $result = $db->query($query) or die ("menu not found");
   $menu_id_result = $result->fetch_array();
   $menu_id = stripslashes($menu_id_result["id"]);

   // create a relationship between this new recipe and the current menu
   $query = "insert into mrrelationship values ('$menu_id','$recipe_id')";
   $db->query($query) or die ("Invalid insert " . $db->error);



?>
